...
bool check(vector<int> &ans, int x, vector<bool> &vis, vector<vector<int>> &v, int i, vector<int> &val)
{
    ans.pb(i);
    x++;
    //  cout<<i<<endl;
    ...
    for (int j = 0; j < v[i].size(); j++)
    {
        ...
        else
        {
            val[v[i][j]] = x;
            vis[v[i][j]] = true;
            if (check(ans, x, vis, v, v[i][j], val))
            {
                return true;
            }
        }
...
void solve(int it)
{
    ...
    for (int i = 0; i < n; i++)
    {
        if (!vis[i])
        {
            ...
            int x = 0;
            val[i] = 0;
            if (check(ans, x, vis, v, i, val))
            {
                int len = 1;
                for (int k = ans.size() - 2; k >= 0; k--)
                {
                    if (ans[k] == ans[ans.size() - 1])
                    {
                        len++ ;
                        break;
                    }
