...
void solve(int it)
{
    ...
    for (ll i = 0; i < m; i++)
    {
        ll a, b;
        cin >> a >> b ;
        v[a - 1].push_back({b - 1, cost[b-1]-cost[a-1]});
    }
    trace(v);
    ...
    while (!s.empty())
    {
        ...
        ll pt = (*s.begin()).second;
        s.erase({cost, pt});
        for (ll i = 0; i < v[pt].size(); i++)
        {
            ll to_pt = v[pt][i].first;
            ll at_cost = v[pt][i].second;
           
            if (at_cost + cost < dist[to_pt])
            {

               
                    s.erase(s.find(make_pair(dist[to_pt], to_pt)));
             

                dist[to_pt] = at_cost + cost;
                s.insert({dist[to_pt], to_pt});
            }
        }
    }
  int q;
