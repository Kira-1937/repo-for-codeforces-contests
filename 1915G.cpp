...
void solve(int it)
{
   ...
   vector<pair<int,int>>time(n,{1e9,1e9});
   time[0]={0,1e9};
//    trace(v,time);
   ...
   int  cyc=cycle[0];
    while(!pq.empty()){
        ...
        for(auto it:v[node]){
            ...
            if(cost+wt*cyc<=time[pt].first){
                 time[pt].first=cost+wt*cyc;
                    trace(time[pt].first,cyc,node+1,pt+1);
                 pq.push({time[pt].first,pt});
            }
            else if(cyc<time[pt].second&&pt!=n-1){
                time[pt].second=cyc;
                time[pt].first=cost+wt*cyc;
                 pq.push({time[pt].first,pt});
            }
