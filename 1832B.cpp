...
int  knap(vector<int>&v,int s,int e,int k){
    if(k==0||s>e){
        return 0;
    }
 
return min(v[s]+v[s+1]+knap(v,s+2,e,k-1),v[e]+knap(v,s,e-1,k-1));
...
long long  minsum=min(v2[k-1],v1[k-1]);
if(k>1){
    for( i=0;i<k-1;i++){
        minsum=min(minsum,v1[i]+v2[k-1-i-1]);
