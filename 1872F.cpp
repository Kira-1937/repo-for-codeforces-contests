...
void solve(vector<int>&dar,vector<int>&indeg,int &ans,vector<int>&res){
    // trace(ans);
    while(!q.empty()){
...
while(!price.empty()){
   ...
   indeg[pt]=0;
    ...
    if(indeg[dar[pt]]==0){
        res.pb(pt);
        q.push(dar[pt]);
        solve(dar,indeg,ans,res);
    }

}
// trace(price,indeg,ans);
trace(res);
