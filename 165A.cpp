...
int count=0;
     ...
     forn (i,n){
        ...
        bool tempd=false;
    forn(j,n){
        if(v[i][0]==v[j][0]&&v[j][1]>v[i][1])tempr=true;
        if(v[i][0]==v[j][0]&&v[j][1]<v[i][1])templ=true;
         if(v[i][0]>v[j][0]&&v[j][1]==v[i][1])tempd=true;
          if(v[i][0]<v[j][0]&&v[j][1]==v[i][1])tempu=true;
    }

    if(templ&&tempu&&tempr&&tempd){
        count++;
    }
