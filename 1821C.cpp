...
for(int i=0;i<s.size();i++){
    if(mp.find(s[i])!=mp.end()){
       ...
       if(mp[s[i]].second<i-mp[s[i]].first-1){
        mp[s[i]].second=i-mp[s[i]].first-1;
        mp[s[i]].first=i;
       }
       else {
        mp[s[i]].first=i;
...
for(auto it:mp){
    // trace(it.second.second,n-1-it.second.first);
    if(it.second.second<n-1-it.second.first){
        // it.second.second=n-1-it.second.first;
        mp[it.first].second=n-1-it.second.first;
    }
}
