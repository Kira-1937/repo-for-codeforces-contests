...
{ios_base::sync_with_stdio(false);cin.tie(0);cout.tie(0);cout<<setprecision(25);
    ...
    vector<vector<pair<int,int>>>v(n,v1);
    for(int i=0;i<m;i++){
        int a,b,cost;
        cin>>a>>b>>cost;
        a--,b--;
...
pair<ll,ll>p={0,0};
q.push(p);
vector<ll>dist1(k,1e17);
vector<vector<ll>>dist(n,dist1);
dist[0][0]=0;
...
while(!q.empty()){
    ...
    if(dist[a][k-1]<cost){
        ...
        for(int i=0;i<v[a].size();i++){
            int b=v[a][i].first;
            int edge_length=v[a][i].second;
            if(dist[b][k-1]>cost+edge_length){
            dist[b][k-1]=cost+edge_length;
            q.push({dist[b][k-1],b});
